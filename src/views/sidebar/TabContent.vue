  ...
  <div :class="{ 'icon-loading': loading }">
    <div v-show="!loading">
      ...
      <EmptyPropertiesPlaceholder
        v-if="properties.knownProperties.length === 0"
      />
  ...
  methods: {
    ...
    async update() {
      ...
      if (!isEmptyObject(this.fileInfo_)) {
        ...
        const customPropertyNames = customProperties.map(
          (cp) => `${cp.prefix}:${cp.propertyname}`
        );

        this.properties.knownProperties = customProperties.map((cp) => {
          const property = properties.find(
            (p) => `${cp.prefix}:${cp.propertyname}` === p.propertyname
          );
          return {
            ...property,
            ...cp,
        ...
        const ownerUrl = generateUrl(
          "/apps/customproperties/customproperties/owner/" + this.fileId
        );
        const ownerId = await axios.get(ownerUrl);

        this.uploaderId = ownerId.data;
        this.loading = false;
      }
    },
    ...
    async retrieveProps(fileInfo) {
      try {
        ...
        const path = `/files/${uid}/${fileInfo.path}/${fileInfo.name}`.replace(
          /\/+/gi,
          "/"
        );
